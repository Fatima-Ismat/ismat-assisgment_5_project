...
if "failed_attempts" not in st.session_state:
     st.sessions_state.failed_attempts = 0
...
def generate_key(passkey):
     key = pbkdf2_hmac('sha256', passkey.emcode(), SALT, 100000)
...
# === user registration ===
     ...
     password = st.text_input("Chose Password", type = "password")
          ...
          if st.button('Login'):
               if username in stored_data and stored_data[username]["password"] == hash_password(password):
                    ...
                    if st.session_state.failed_attempts >= 3:
                         ...
                         st.error("🛑 To many failed attenpts. Locked for 60 seconds")
                         st.stop()
     ...
     if not st.session_state.authenticated_user:
          ...
          if st.button("Encrypt And Save"):
               if data and passkey:
                    ...
                    stored_data[st.session_state.__annotations__user]["data"].append(encrypted)
                    save_data(stored_data)
